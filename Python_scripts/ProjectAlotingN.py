...
script_dir=os.path.dirname(os.path.abspath('ProjectAlotingN.py'))
inputfolder=os.path.join(script_dir,'..','Residuals')
# Read the students and projects files into DataFrames
studentsNpath=os.path.join(inputfolder,'Student N Citizenship_.xlsx')
projectsNpath=os.path.join(inputfolder,'Projects for N Citizens.xlsx')
students_df =   pd.read_excel(studentsNpath)
projects_df =   pd.read_excel(projectsNpath)
...
outputAssignedpath=os.path.join(inputfolder,'assigned_projects.xlsx')
assigned_projects_df.to_excel(outputAssignedpath, index=False)


# Combine the DataFrames vertically
# combined_df = pd.concat([students_df, Noncitizen_df], ignore_index=True)

# # Save the updated student DataFrame to a new Excel file
remainingStudents=os.path.join(inputfolder,'RemainingStudents.xlsx')
students_df.to_excel(remainingStudents, index=False)
